...
- **Modern Vue 3.5+ Reactivity Idioms:**
  ...
  - **Options Normalization & Default Values:**
    - **Dynamic Options (`MaybeRefOrGetter<T>`):** Normalize into a `computed` once at the top of their feature block or composable setup scope (e.g., `const isEnabled = computed(() => toValue(options.enabled) ?? true);`). Unwrapping must always be sequenced as `toValue(options.prop) ?? default` (never `toValue(options.prop ?? default)`). Never inline `toValue(options.prop ?? default)` inside event handlers, watchers, or internal callbacks.
    - **Non-Reactive / Static Options:** Extract once via object destructuring with default values at the top of the composable (e.g., `const { event = "pointerdown", capture = true } = options;`). Never query static options with ad-hoc inline defaults.
...
- [ ] Proportional, scope-aware validation — run the narrowest command that can actually falsify the change:
  - **Re-runs are expected whenever the revision or the question changed**: a baseline run before editing (is the workspace already red?), a regression run after the behavioral change, and a confirmation run after adding, editing, or fixing a test are each legitimate — a red → fix → green cycle is three runs of the same suite by design. What is redundant: the same suite twice against an **unchanged** revision, a broader suite (for example `pnpm test:run`) when the targeted spec already covers the diff, or a re-run purely to re-read output you already have.
  - **Non-executable edits** (code comments, JSDoc prose that does not change a signature or type, `README`/`AGENTS.md`/`.agents/**` markdown, `RFC/` and `plan/` notes, formatting-only whitespace): **run no tests**. Say plainly that validation was not applicable rather than implying coverage. Note that `oxlint` and `oxfmt` are both scoped to `packages/vue/src` and `playground/src`, so prose, rule, and config files are outside lint scope too — running them there proves nothing.
  - **Behavioral changes in `packages/vue/src/`** (logic, reactive wiring, DOM/event handling, types, public API): `pnpm lint` plus the **targeted** spec for the touched feature (for example `vitest run use-outside-click`); add `pnpm run test:ssr` when SSR, scope teardown, or cross-realm document/window/timer resolution is involved.
  - **Cross-cutting or pre-release changes** (shared modules such as `packages/vue/src/shared/`, public types, several features at once, build or dependency config): the full set — `pnpm lint`, `pnpm run test:ssr`, `pnpm test:run`.
  - For `docs/` changes: `pnpm docs:lint`, plus `pnpm docs:build` when frontmatter, navigation, config, or embedded components changed.
  - For `playground/` changes: `pnpm --filter v-float-playground build`.
  - Always report which commands were run, and name any that were deliberately skipped as unnecessary.
