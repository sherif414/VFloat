...
- [`useFocusTrap`](/api/use-focus-trap) handles the entire surface focus lifecycle: routing initial focus inside on open, trapping or wrapping <kbd>Tab</kbd> navigation, isolating outside content with `inert`, and returning focus to the trigger on close.
