...
- [x] `useTree` exposes a cleanup function or equivalent teardown handle.
- [x] The internal watchers created by `useTree` stop cleanly when cleanup runs.
- [x] Existing component-scoped usage still works without extra caller burden.
- [x] The cleanup story is documented in the tree return type or inline source comments, whichever best fits the repo’s existing style.
...
Fully completed and verified via extensive unit tests. Both manual lifecycle cleanup and automated effect scope teardown have been verified successfully.
