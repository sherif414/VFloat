...
- [x] `getFlattenedItems()` derives visible items from the model’s indexed child structure.
- [x] The flattening path no longer calls `getItemChildren` a second time.
- [x] Flattening output remains identical for valid input trees.
- [x] Tree-model invariants continue to be covered by tests.
...
Fully completed and verified via mock/spy unit tests. Flattening now traverses `childrenItemsMap` exclusively, completely bypassing the redundant `getItemChildren` callback calls.
