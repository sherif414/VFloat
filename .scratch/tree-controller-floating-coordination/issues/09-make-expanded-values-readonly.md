...
- [x] `expandedValues` is exposed as a readonly ref in the public tree return type.
- [x] Consumers cannot mutate the expansion set in place through the public interface.
- [x] Existing tree mutators continue to update expansion state reactively.
- [x] Type coverage or tests demonstrate that expansion state must be changed through tree methods.
...
Fully completed and verified. `expandedValues` is now returned as a Vue `readonly(...)` ref proxy typed as `Ref<ReadonlySet<string>>`. Direct mutations are blocked at both compile-time and runtime, and expansion operates reactively via tree methods.
